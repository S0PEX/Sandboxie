...
void CShellDialog::SyncRunLogon()
{
    ...
    if (ini) {
        ...
        STARTUPINFO si;
        if (AvastSboxDll_RunFromHome(SBIECTRL_EXE, NULL, &si, NULL)) {
...
void CShellDialog::CreateAssoc(WCHAR *path, const WCHAR *classname)
{
    ...
    if (rc == 0 && AvastSboxDll_RunFromHome(START_EXE, NULL, &si, NULL)) {
    ...
    if (AvastSboxDll_RunFromHome(START_EXE, path, &si, NULL)) {
    ...
    const CString &setting, int nFolder, const CString &subdir)
{
    ...
    if (ini) {
        ...
        STARTUPINFO si;
        if (AvastSboxDll_RunFromHome(L"QuickLaunch.lnk", NULL, &si, NULL)) {
            WCHAR *path2 = (WCHAR *)si.lpReserved;
    ...
    BOOL run_elevated)
{
    ...
    if (! StartExe) {
        ...
        STARTUPINFO si;
        if (AvastSboxDll_RunFromHome(START_EXE, L"", &si, NULL)) {
