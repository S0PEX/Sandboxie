...
CString CSbieIni::GetPath(BOOL *IsHomePath)
{
    ...
    if (req) {
        ...
        SBIE_INI_GET_PATH_RPL *rpl =
            (SBIE_INI_GET_PATH_RPL *)AvastSboxDll_CallServer(&req->h);
        if (rpl) {
...
BOOL CSbieIni::GetUser(CString &Section, CString &Name, BOOL &IsAdmin)
{
    ...
    if (req) {
        ...
        SBIE_INI_GET_USER_RPL *rpl =
            (SBIE_INI_GET_USER_RPL *)AvastSboxDll_CallServer(&req->h);
        if (rpl) {
            if (rpl->h.status == 0) {
                if (rpl->admin)
                    IsAdmin = TRUE;
    ...
    const CString &SectionName, const CString &SettingName)
{
    ...
    while (1) {
        ...
        MSG_HEADER *rpl = AvastSboxDll_CallServer((MSG_HEADER *)RequestBuf);
        if (rpl) {
            status = rpl->status;
            AvastSboxDll_FreeMem(rpl);
        } else
...
void CSbieIni::MonitorService(CWnd *pWnd, ULONG idExitCmd)
{
    ...
    SBIE_INI_GET_WAIT_HANDLE_RPL *rpl =
        (SBIE_INI_GET_WAIT_HANDLE_RPL *)AvastSboxDll_CallServer(&req.h);
    if (rpl) {
        if (rpl->h.status == 0) {
            ULONG ThreadId;
