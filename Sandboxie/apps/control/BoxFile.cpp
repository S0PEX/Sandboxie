...
CString CBoxFile::GetCopyPathForTruePath(const WCHAR *TruePath) const
{
    ...
    for (int index = SeparateUserFolders ? -4 : -1; index < 26; ++index) {
        ...
        if (index == -4)
			prefix = AvastSboxDll_GetUserPathEx(L'p');
        else if (index == -3)
            prefix = AvastSboxDll_GetUserPathEx(L'a');
        else if (index == -2)
            prefix = AvastSboxDll_GetUserPathEx(L'c');
        else if (index == -1)
			prefix = L"\\device\\mup"; 
        else
            prefix = AvastSboxDll_GetDrivePath(index);
        if (! prefix)
            continue;
...
CString CBoxFile::GetDriveNameForTruePath(const WCHAR *TruePath) const
{
    ...
    for (int index = 0; index < 26; ++index) {
        ...
        const WCHAR *prefix = AvastSboxDll_GetDrivePath(index);
        if (! prefix)
            continue;
