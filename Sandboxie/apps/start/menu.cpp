...
_FX HMENU BuildTopLevelMenu(MENU_DIR *root)
{
    ...
    if (! _desktop)
        _desktop = AvastSboxDll_FormatMessage0(MSG_3112);
...
#define GET_FOLDER_PATH(csidl) \
        ...
        while (1) {
            ...
            if ((data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
                ...
                if (_wcsicmp(name, L"Programs") == 0)
                    wcscpy(name, AvastSboxDll_FormatMessage0(MSG_3114));
    ...
    if (! _all_files_and_folders)
        _all_files_and_folders = AvastSboxDll_FormatMessage0(MSG_3521);
...
_FX void BuildMenu(HMENU hMenu, MENU_DIR *menu, WCHAR *fullpath)
{
    ...
    if (! _explore)
        _explore = AvastSboxDll_FormatMessage0(MSG_3113);
    if (! _cancel)
        _cancel  = AvastSboxDll_FormatMessage0(MSG_3002);
    ...
    if (! fullpath) {
        ...
        wsprintf(
            title, AvastSboxDll_FormatMessage1(MSG_3111, boxname));
    ...
    while (1) {
        ...
        if (wcscmp(data.cFileName, L".") != 0 &&
            ...
            if (hFile != INVALID_HANDLE_VALUE) {
                ...
                BOOLEAN IsBoxedPath;
                LONG rc = AvastSboxDll_GetHandlePath(
...
_FX void ScanAllFilesAndFolders(MENU_DIR *menu, WCHAR *path)
{
    if (menu->path[0]) {
        ...
        for (DriveLetter = L'A'; DriveLetter <= 'Z'; ++DriveLetter) {
            ...
            DrivePath[1] = L'\0';
            DriveTitle = AvastSboxDll_FormatMessage1(MSG_3676, DrivePath);
            DrivePath[1] = L':';
            DrivePath[2] = L'\\';
            DrivePath[3] = L'\0';
...
_FX WCHAR *DoStartMenu()
{
    ...
    title = AvastSboxDll_FormatMessage0(MSG_3465);
    ...
    if (GetLinkIconPathAndNumber(buf + 64, &IconPath, &IconIndex)) {
        ...
        if (IconPath[0]) {
            ...
            if (hFile != INVALID_HANDLE_VALUE) {
                ...
                BOOLEAN IsBoxedPath;
                if (0 == AvastSboxDll_GetHandlePath(
                                    hFile, buf + 1024, &IsBoxedPath)) {
                    if (AvastSboxDll_TranslateNtToDosPath(buf + 1024)) {
