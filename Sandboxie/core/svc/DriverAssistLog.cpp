...
void DriverAssist::LogMessage_Single(ULONG code, wchar_t* data)
{
    ...
    if (! AvastSboxDll_GetServiceRegistryValue(L"LogFile", &u.info, sizeof(u)))
        return;
    ...
    WCHAR *text = AvastSboxDll_FormatMessage2(code, str1, str2);
    if (! text)
        return;
    ...
    ULONG msgid, const WCHAR *path, const WCHAR *text)
{
    ...
    if (! AvastSboxDll_GetServiceRegistryValue(L"MultiLog", &u.info, sizeof(u)))
        return;
    if (u.info.Type != REG_SZ || u.info.DataLength >= sizeof(u))
        return;
