...
_FX BOOLEAN Custom_CreateRegLinks(void)
{
    ...
    if (status == STATUS_ACCESS_DENIED) {
        ULONG mp_flags = AvastSboxDll_MatchPath(L'k', path);
    ...
    while (1) {
        ...
        if (status == STATUS_OBJECT_NAME_NOT_FOUND) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                AvastSboxDll_ExpandAndRunProgram(buf2);
...
//---------------------------------------------------------------------------
// AvastSboxDll_ExpandAndRunProgram
//---------------------------------------------------------------------------


_FX BOOLEAN AvastSboxDll_ExpandAndRunProgram(const WCHAR *Command)
{
    ULONG len;
    WCHAR *cmdline;
    STARTUPINFO si;
...
// it does, after processing by AvastSboxDll_Hook), this is not fixed up while
...
_FX void Custom_ComServer(void)
{
    ...
    // AvastSboxDll_RunSandboxed request which runs an instance of SbieSvc
    ...
    if (Proc_ImpersonateSelf(TRUE)) {
        ...
        BOOL ok = AvastSboxDll_RunSandboxed(L"*THREAD*", L"*COMSRV*", L"", 0,
