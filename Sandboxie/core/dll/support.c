...
//---------------------------------------------------------------------------
// Support Functions for Programs Using AvastSboxDll
//---------------------------------------------------------------------------
...
static void AvastSboxDll_SetStartError(ULONG Level);
...
static WCHAR *AvastSboxDll_StartError = NULL;
...
//---------------------------------------------------------------------------
// AvastSboxDll_SetStartError
//---------------------------------------------------------------------------


_FX void AvastSboxDll_SetStartError(ULONG Level)
{
    ...
    if (AvastSboxDll_StartError) {
        Dll_Free(AvastSboxDll_StartError);
        AvastSboxDll_StartError = NULL;
    }
    ...
    AvastSboxDll_StartError = Dll_Alloc(len * sizeof(WCHAR));

    Sbie_snwprintf(AvastSboxDll_StartError, len,
...
//---------------------------------------------------------------------------
// AvastSboxDll_GetStartError
//---------------------------------------------------------------------------


_FX const WCHAR *AvastSboxDll_GetStartError(void)
{
    return AvastSboxDll_StartError;
}


//---------------------------------------------------------------------------
// AvastSboxDll_StartSbieSvc
//---------------------------------------------------------------------------


_FX BOOLEAN AvastSboxDll_StartSbieSvc(BOOLEAN retry)
{
    ...
    for (retries = 0; retries < 3; ++retries) {
        ...
        rpl = (SBIE_INI_GET_VERSION_RPL *)AvastSboxDll_CallServer(&req.h);
        if (rpl) {
            Dll_Free(rpl);
            return TRUE;
        }
        ...
        if (retries == 0) {
            ...
            if (pOpenSCManagerW) {
                ...
                if (! hScm)
                    AvastSboxDll_SetStartError(0x11);
            }
            ...
            if (hScm) {
                ...
                if (pOpenServiceW) {
                    ...
                    if (! hSvc)
                        AvastSboxDll_SetStartError(0x22);
                    ...
                    if (pStartServiceW) {
                        ...
                        if (! ok)
                            AvastSboxDll_SetStartError(0x33);
                    }
...
//---------------------------------------------------------------------------
// AvastSboxDll_GetServiceRegistryValue
//---------------------------------------------------------------------------


_FX BOOLEAN AvastSboxDll_GetServiceRegistryValue(
...
_FX ULONG AvastSboxDll_GetLanguage(BOOLEAN *rtl)
{
    static ULONG lang = 0;
    union {
    ...
    if (! lang) {
        ...
        if (AvastSboxDll_GetServiceRegistryValue(
...
//---------------------------------------------------------------------------
// AvastSboxDll_KillOne
//---------------------------------------------------------------------------


_FX BOOLEAN AvastSboxDll_KillOne(ULONG ProcessId)
{
...
//---------------------------------------------------------------------------
// AvastSboxDll_KillAll
//---------------------------------------------------------------------------


_FX BOOLEAN AvastSboxDll_KillAll(ULONG SessionId, const WCHAR *BoxName)
{
...
//---------------------------------------------------------------------------
// AvastSboxDll_GetTokenElevationType
//---------------------------------------------------------------------------


_FX ULONG AvastSboxDll_GetTokenElevationType(void)
{
...
//---------------------------------------------------------------------------
// AvastSboxDll_FormatMessage_2
//---------------------------------------------------------------------------

extern int __CRTDECL Sbie_snwprintf(wchar_t *_Buffer, size_t Count, const wchar_t * const _Format, ...);

_FX ULONG AvastSboxDll_FormatMessage_2(WCHAR **text_ptr, const WCHAR **ins)
{
    //
...
//---------------------------------------------------------------------------
// AvastSboxDll_FormatMessage
//---------------------------------------------------------------------------


_FX WCHAR *AvastSboxDll_FormatMessage(ULONG code, const WCHAR **ins)
{
    ...
    if (! SbieMsgDll) {
        ...
        STARTUPINFOW si;
        if (AvastSboxDll_RunFromHome(SBIEMSG_DLL, NULL, &si, NULL)) {
    ...
    if (SbieMsgDll) {
        ...
        rc = FormatMessage(FormatFlags, SbieMsgDll, code,
                           AvastSboxDll_GetLanguage(NULL),
                           (LPWSTR)&out, 4, (va_list *)ins);
        if (rc != 0) {
            ULONG xrc = AvastSboxDll_FormatMessage_2(&out, ins);
            if (xrc)
                rc = xrc;
        }
    }
...
//---------------------------------------------------------------------------
// AvastSboxDll_FormatMessage0
//---------------------------------------------------------------------------


_FX WCHAR *AvastSboxDll_FormatMessage0(ULONG code)
{
    return AvastSboxDll_FormatMessage(code, NULL);
}


//---------------------------------------------------------------------------
// AvastSboxDll_FormatMessage1
//---------------------------------------------------------------------------


_FX WCHAR *AvastSboxDll_FormatMessage1(ULONG code, const WCHAR *ins1)
{
    const WCHAR *ins[6];
    memzero((WCHAR *)ins, sizeof(ins));
    ins[1] = (WCHAR *)ins1;
    return AvastSboxDll_FormatMessage(code, ins);
}


//---------------------------------------------------------------------------
// AvastSboxDll_FormatMessage2
//---------------------------------------------------------------------------


_FX WCHAR *AvastSboxDll_FormatMessage2(
