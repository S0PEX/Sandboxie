    ...
    PROCESS *proc, HANDLE ProcessHandle, void *InfoBuffer, ULONG InfoLength)
{
    ...
    NTSTATUS status;
    KIRQL irql;
    BOOLEAN SpecialCallFromAvastSboxDll;
    ...
    SpecialCallFromAvastSboxDll = FALSE;
    __try {
        status = STATUS_SUCCESS;
        if (InfoLength == sizeof(PROCESS_ACCESS_TOKEN)) {
            ...
            if ((! Info->Token) && (! Info->Thread))
                SpecialCallFromAvastSboxDll = TRUE;
        }
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        status = GetExceptionCode();
    }
    ...
    if (proc2 && (! proc2->initialized)) {
        ...
        // in AvastSboxDll, and in this case, InfoBuffer should contain zeroes,
        // and SpecialCallFromAvastSboxDll will be set by the code sequenece above
        //

        if (! SpecialCallFromAvastSboxDll) {
            ...
            //
            // the special call from AvastSboxDll is a request to copy the active
...
{
    //
    // the call to SetProcessWindowStation by AvastSboxDll during process
